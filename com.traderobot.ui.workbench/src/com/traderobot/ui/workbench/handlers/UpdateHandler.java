...
//	private static final String REPOSITORY_LOC = System.getProperty("UpdateHandler.Repository", "http://localhost/repository");	
	private static String REPOSITORY_LOC = System.getProperty("UpdateHandler.Repository", "file:////F:/deploy/repository");
	
	@Execute
    public void execute(final IProvisioningAgent agent, final Shell shell, final UISynchronize sync, final IWorkbench workbench) {
		
		InputDialog dialog = new InputDialog(shell, "Update Location", "Update Location을 입력하십시오.", REPOSITORY_LOC, null);
		if ( dialog.open() == InputDialog.OK )
			REPOSITORY_LOC = dialog.getValue();
    ...
    private IStatus checkForUpdates(final IProvisioningAgent agent, 
        ...
        configureUpdate(shell, operation);
...
//        	logger.error("Trying to update from the Eclipse IDE? This won't work!");
    ...
    private UpdateOperation configureUpdate(final Shell shell, final UpdateOperation operation) {
